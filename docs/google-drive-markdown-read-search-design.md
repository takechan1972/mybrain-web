...
> **ステータス：Phase 1 実装済み・本番QA完了（OBS26／OBS26R）／Phase 2 実装済み・本番QA待ち（2026-07-09・OBS27）／Phase 3 は未着手。**
...
| # | ケース | 期待 | 合否 |
|---|---|---|---|
| R5 | 1件プレビュー | 一覧の「内容を確認」→ 選んだファイルのタイトル・タグ・作成/更新日時・本文が正しく表示される（frontmatter 復元）。「閉じる」でプレビューが消える | ☐ Pass ☐ Fail |
| R6 | frontmatter 無しファイル | エラーにならず、本文がそのまま表示され「MyBrain形式の情報が見つからなかった」旨の注記が出る。タイトル位置にはファイル名が表示される | ☐ Pass ☐ Fail |
| R8 | 保存しないこと | プレビュー後にページを再読み込みすると一覧・プレビューは消えている（Supabase・localStorage に保存されない）。プレビューに保存・取り込み・編集ボタンが無い | ☐ Pass ☐ Fail |
| P1 | 読み込み中表示 | 読み込み中は「読み込み中...」が表示される | ☐ Pass ☐ Fail |
| P2 | 読み込み失敗 | 通信失敗等では「内容を読み込めませんでした。もう一度お試しください。」と表示される | ☐ Pass ☐ Fail |
| P3 | 読み取り専用であること | プレビューを開いても Drive のファイルは変更・削除されない（更新日時が変わらない） | ☐ Pass ☐ Fail |
