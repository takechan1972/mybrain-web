...
> **ステータス：Phase 1 実装済み・本番QA完了（OBS26／OBS26R）／Phase 2 実装済み・本番QA完了（OBS27／OBS27R）／Phase 3a 実装済み・本番QA待ち（2026-07-09・OBS29）／Phase 3b は未着手。**
...
> Phase 3a（検索参照・AI非接続）は実装済み・本番QA待ち（2026-07-09・OBS29）。Phase 3b（AI参照）は未実装。詳細は下の「Phase 3 詳細設計（OBS28）」。OAuth スコープは `drive.file` のまま変更なし。
...
1. **Phase 3a（検索参照）— ✅実装済み（2026-07-09・OBS29）**：「参照に追加」＋ 参照中表示 ＋ 検索結果の独立セクション「Google Drive参照の検索結果」（バッジ付き・1件解除／すべて解除）。AI には接続しない。
...
| # | ケース | 期待 | 合否 |
|---|---|---|---|
| S1 | 参照に追加 | デスクトップのメモ一覧 ＞「エクスポート済み一覧」＞「一覧を確認」→ 各ファイルの「参照に追加」で読み込まれ、「◯◯を参照に追加しました。」の案内が出る | ☐ Pass ☐ Fail |
| S2 | 重複追加なし | 追加済みファイルはボタンが「参照中」になり、二度追加できない | ☐ Pass ☐ Fail |
| S3 | 参照解除 | 検索結果セクションの「参照を解除」で1件ずつ外せる。「すべて解除」で全て外れる | ☐ Pass ☐ Fail |
| S4 | 検索に出る | 検索語に一致する参照メモが「Google Drive参照の検索結果（N件）」に「Google Drive参照」バッジ付きで表示される。本体メモの一覧・件数・フォルダ・お気に入りには混ざらない | ☐ Pass ☐ Fail |
| S5 | 検索に出ない | 参照0件、または検索語が空のとき、参照セクション自体が表示されない（既存画面は不変） | ☐ Pass ☐ Fail |
| S6 | 保存されない | ページを再読み込みすると参照は消える（Supabase・localStorage に残らない） | ☐ Pass ☐ Fail |
| S7 | 読み取り専用 | 参照メモに本体メモ用の編集・削除・お気に入り・フォルダ・エクスポート操作が無い。Drive のファイルも変更されない | ☐ Pass ☐ Fail |
| S9 | AI非接続 | AI アシスタントに質問しても、参照メモは AI コンテキストに含まれない（Phase 3a では未接続） | ☐ Pass ☐ Fail |
| S10 | モバイル不変 | モバイル UI に参照機能が表示されず、挙動も変わらない | ☐ Pass ☐ Fail |
