...
- 次フェーズ案（すべて未着手）：A. ヘルプ/設定への文言反映、B. モバイルへの参照・AI 拡張、C. 参照の使い勝手改善、D. 取り込み（インポート）機能の検討（要設計）、E. 検索の高度化（将来）。→ D は OBS43 で設計完了（`docs/memo-import-design.md`・実装は IMP1 以降・2026-07-18）。
...
### OBS43：メモ取り込み（インポート）機能 設計（IMP0）— ✅設計完了・未実装（2026-07-18）

- OBS32 次フェーズ D「取り込み（インポート）機能の検討（要設計）」の設計を実施した（**ドキュメントのみ・アプリコード変更なし**）。
- 新規追加：`docs/memo-import-design.md`（全22セクション・OBS43）。
- スコープ（Phase 1）：**個別 Markdown ファイル（.md）の取り込み・複数選択対応・1回最大20件**。21件以上の選択は案内を出して開始しない。
- 中核ルール（承認済み）：
  - **MyBrain（Supabase）が正本（source of truth）**。取り込みは Supabase への追加のみ。
  - **取り込みメモは必ず新しい Supabase メモIDを採番**する（frontmatter の id は insert に使わない）。
  - **frontmatter の元 id は重複検知・由来（provenance）表示のみ**に使う。
  - **有効な created / updated は保持**する。created 無効・欠落→取り込み時刻／updated 無効・欠落→保持した created（無ければ取り込み時刻）／updated < created は無効扱い→created。
  - **insert-only・サイレント上書きなし**（既存メモの更新・削除経路を持たない）。重複（同じメモID）は既定スキップ、タイトル＋本文完全一致の「重複の可能性」は既定スキップ＋明示選択時のみ新規取り込み可。
  - **プレビューと明示的な確認が必須**（自動保存なし・10件以上は既存の警告しきい値を再利用）。
  - **保存先は Supabase のみ**（保存先設定 mybrain / obsidian-local / obsidian-gdrive のどれでも同じ）。
  - **Obsidian ローカル・Google Drive への自動書き戻しはしない**。
- 後回しフォーマット：ZIP（IMP4）／MyBrain JSON（エクスポート形式の設計とセットで別 OBS）／CSV（不採用の方向）。
- `ImportCandidate` 型案・1MB/件の上限・部分失敗の正直な報告（ロールバックなしを明記）・RLS／プライバシー境界を設計に含めた。
...
- 現状：
  ...
  - 実装コミット：`14fbf9d feat: add memo import pure helpers (IMP1)`（push 済み）。
- 既知の制限（設計どおり）：
...
- 次フェーズ：**IMP2 デスクトップ取り込み UI**（ファイル選択 → 候補プレビュー → 明示的な確認 → Supabase insert ループ → 結果サマリ）を別の独立タスクとして扱う。→ プレビューのみの IMP2a は OBS45 で実装・QA 完了（2026-07-19）。保存接続（IMP2b）は未着手。

### OBS45：メモ取り込み（インポート）IMP2a デスクトップ・プレビューUI実装 — ✅実装完了・デスクトップ手動QA Pass（2026-07-19）

- OBS43 設計の IMP2 のうち、**プレビュー（確認）のみ**を IMP2a として実装した。**保存経路は一切なし**：Supabase への insert・update・delete は呼ばず、`createMemoWithTimestamps`／`createMemo` は import もしていない（コード検索で確認済み）。
- 変更ファイル：
  - 新規：`components/MemoImportPanel.tsx`（デスクトップ専用の取り込みプレビューパネル）。
  - 更新：`components/DesktopSettings.tsx`（データ管理タブ末尾にパネルを配置。「⬇ インポート（データ取込）」行はトースト「準備中です」からデータ管理タブへの切替＋取り込みセクションへのスクロールに接続。カード副題を「保存データの確認・エクスポート・取り込み」に更新）。
  - 更新：`docs/PROJECT_STATUS.md`（OBS44 の古い「コミット・push は未実施」記載を実コミット `14fbf9d`（push 済み）に修正＋本エントリ）。
- 実装内容（設計 §3・§12・§18 のプレビュー部分）：
  - `<input type="file" multiple accept=".md,text/markdown">` による複数選択（モーダルは使わない・データ管理タブ内の専用セクション）。
  - **1回最大20件**（`IMPORT_MAX_FILES_PER_BATCH`）：21件以上はやさしい案内を出して**一切処理しない**（部分処理しない）。
  - **1ファイル1MB**（`IMPORT_MAX_FILE_BYTES`）：検証は既存 IMP1 ヘルパー（`buildImportCandidate`）に委譲。
  - IMP1 純ヘルパーを再利用：`buildImportCandidate` → `detectImportDuplicates` → `setImportAsNew` → `listImportTargets`（ロジックの重複実装なし）。
  - 既存メモ一覧（`listMemos`）は**選択バッチごとに1回だけ**取得（読み取り専用・重複検知のみに使用）。取得失敗時は重複確認ができない旨を正直に表示してバッチを中止。
  - ファイル読み取り失敗・検証エラーは**ファイルごとに理由付きで表示**し、他ファイルの処理は継続。
  - プレビュー表示：合計サマリ（選択／取り込み可能／重複（同じメモID）／重複の可能性／エラー件数）・ファイル名・確定タイトル（「無題」フォールバック）・タグ・作成/更新日時（「元のメモの日時」／「取り込み時刻」ラベル付き）・本文冒頭約200字・状態バッジ・理由。
  - **重複（同じメモID）は常にスキップ**（選択肢なし）。**「重複の可能性」のみ「新しいメモとして取り込む」チェックを表示**（既定 OFF・`setImportAsNew` 経由）。チェックしても現段階では保存されない（取り込み対象件数の表示が変わるのみ）。
  - 固定の注意書き：「既存のメモは変更されません。取り込み可能なメモだけを新しいメモとして追加します。」
  - 取り込みアクション欄：ボタン「保存は次のステップで対応」は**常に無効**＋「現在は取り込み内容の確認のみできます。まだMyBrainには保存されません。」を表示。
  - **古い非同期結果の防止**：バッチ世代カウンタ（ref）で、読み取り中に別バッチを選択・クリアした場合は古い結果を破棄。
  - 候補・選択状態は React state のみ（Supabase・localStorage に保存しない）。
- **デスクトップ手動QA（ログイン済み実画面・2026-07-19・Pass）**：
  - 「アカウント・その他」の「⬇ インポート（データ取込）」でデータ管理タブへ切り替わり、「メモの取り込み（インポート）」セクションへスクロールした。
  - 固定の注意書きと「Markdownファイルを選択」ボタンが表示された。
  - エクスポート済み Markdown「トマトの食べ方.md」を選択 → 選択1件／取り込み可能0件／重複（同じメモID）1件／取り込み対象0件と正しく判定された。
  - 理由「同じメモIDのメモが既に存在するためスキップします。」が表示された。
  - 重複（同じメモID）候補に「新しいメモとして取り込む」チェックは表示されなかった。
  - 「保存は次のステップで対応」ボタンは無効で押せなかった。
  - メモは1件も保存・変更されなかった。
- 境界（変更していないもの）：モバイル UI／メモ CRUD・`MemoStore`・`lib/memos.ts` facade／Supabase スキーマ・RLS／OAuth スコープ／Google Drive・Obsidian の挙動／自動保存なし。MyBrain（Supabase）は引き続き source of truth。
- 検証：`npx tsc --noEmit` Pass／`npm run build` Pass（`/settings` のみサイズ増・他ルート不変）／`git diff --check` OK。
- 既知の制限：保存は未接続（IMP2b で対応）。バッチ内（選択ファイル同士）の重複検知は行わない（IMP1 からの既知の限界）。
- 次フェーズ：**IMP2b**（「N件をMyBrainに取り込む」ボタンの有効化 → 件数入り確認ダイアログ・10件以上の段階的警告 → `createMemoWithTimestamps` による insert-only 保存ループ → 結果サマリ）を別の独立タスクとして扱う。着手前に承認を得る。→ IMP2b は OBS46 で実装完了・本番スモークQA Pass（2026-07-19）。

### OBS46：メモ取り込み（インポート）IMP2b 保存フロー接続 — ✅実装完了・本番スモークQA Pass（2026-07-19）

- OBS45（IMP2a・プレビューのみ）に保存フローを接続した。変更は `components/MemoImportPanel.tsx` と本ドキュメントのみ（`DesktopSettings.tsx`・`lib/import/`・`lib/storage/`・モバイル UI は無変更）。
- **保存対象は `listImportTargets(candidates)` の結果のみ**（`ok` 全件＋「新しいメモとして取り込む」を明示選択した「重複の可能性」。選別ルールを UI 側で再実装しない）。確認開始時点の候補配列をスナップショットし、確認・保存中に対象が変わらないようにした。
- **insert-only**：保存は取り込み専用の `createMemoWithTimestamps` のみを使用。**元メモの id（sourceId）・source は決して渡さない**（Supabase が新しいメモIDを採番）。既存メモの update・delete は呼ばない。Obsidian ローカル・Google Drive への書き出しもしない（設計 §14）。自動保存なし。
- 確認フロー（`window.confirm`・既存デスクトップ一括エクスポートと同じ方式・モーダル不使用・設計 §13）：
  - バッチ読み込み後にボタン「**N件をMyBrainに取り込む**」を表示（N＝取り込み対象件数。0件では無効・1件以上で有効）。
  - **10件以上**は先に段階的警告「取り込み件数が多いため、保存に少し時間がかかる場合があります。N件を取り込みます。続けますか？」。
  - 続けて件数入り確認「N件のメモをMyBrainに新しいメモとして取り込みます。既存のメモは変更されません。よろしいですか？」。
  - **どちらかをキャンセルすると1件も insert しない。**
- 保存ループ（設計 §15・§16）：
  - 対象を**1件ずつ順に** insert。各呼び出しを try/catch で包み、`error` 返却も予期しない例外もそのファイルの失敗（ファイル名・タイトル・理由）として記録して**残りを続ける**。memo も error も無い場合は「保存結果を確認できませんでした。」を記録。
  - **1件の失敗で全体を止めない。自動リトライなし。成功済み insert のロールバックなし**（結果を正直に表示）。
- **二重送信防止**：state ガード（`loaded` のときだけ実行開始）＋保存中はボタン・ファイル選択・「選択をクリア」・「重複の可能性」チェックボックスをすべて無効化。**完了（done）後は取り込みボタンを非表示**にし、同じバッチの再送信を構造的に不可にした（再取り込みは選び直し→最新メモ一覧で重複を再判定）。
- **進捗表示**：「取り込み中… 3 / 10件」形式で1件ごとに更新。保存中もプレビューカードは表示を保つ。
- **結果サマリ（トーストではなくパネル内に常設**・「選択をクリア」か新しい選択まで表示継続）：
  - 追加 X件・スキップ Y件・失敗 Z件（Y＝重複（同じメモID）＋未選択の「重複の可能性」＋エラー の合計。最終スナップショットから算出）。
  - スキップ内訳（3分類）と、失敗のファイル名・タイトル・理由の一覧。
  - 成功＋失敗の混在時：「一部のメモは追加済みです。追加済みのメモはそのまま残ります。失敗した分だけ、ファイルを選び直してやり直せます。」／全件成功：完了を明示／全件失敗：「メモは1件も追加されませんでした。」
- **アンマウント・古い操作の保護**：バッチ世代（ref）を `useEffect` クリーンアップで無効化。各 insert の**開始前**と **await 後**に世代を確認し、無効なら**次の insert を始めずに停止**（実行中の insert は完了を待つ・保存済みメモは残す・アンマウント後の setState はしない）。タブ切替のブロックは追加しない。
- 画面間のメモ一覧更新の配線は追加しない（「追加したメモは、メモ画面を開くと一覧に表示されます。」の案内文のみ）。
- MyBrain（Supabase）は引き続き source of truth。モバイル UI は無変更。
- 検証：`npx tsc --noEmit` Pass／`npm run build` Pass／`git diff --check` OK／コード検索で `createMemo`（無印）・update・delete・Drive/Obsidian 書き出し関数の不使用と、sourceId を insert に渡していないことを確認。テストデータの投入なし。
- **デスクトップ本番スモークQA（本番モード・ログイン済み実画面・ユーザー実施・2026-07-19・Pass）**：
  - 新しい Markdown ファイルが「取り込み可能」として判定された。
  - 取り込みボタン（「N件をMyBrainに取り込む」）が有効になった。
  - 取り込みが正常に完了した。
  - 結果サマリ：追加 1件・スキップ 0件・失敗 0件。
  - 取り込んだメモがメモ一覧に表示された。
  - タイトル・本文が正しく表示された。
  - 元のメモの作成日時・更新日時が保持されていた。
  - 同じメモIDのファイルは「重複（同じメモID）」として正しくスキップされた。
  - 既存のメモは1件も上書きされなかった。
- **設計 §20 の I1〜I19 全項目QAは IMP3 として別タスクで実施する（未着手）。**
