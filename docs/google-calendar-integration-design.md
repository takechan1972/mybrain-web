...
## 本番検証ログ（Production verification log）

> 実装済み機能の本番確認記録。設計メモ本文（上記）は方針であり、以下は実際の動作確認結果。

### CAL3R：読み取り専用「今日の予定」表示（read-only today display）

- **検証日**：2026-07-03
- **環境**：
  - Vercel 本番（production）
  - モバイル予約画面（`app/reservations/page.tsx`）
  - Google カレンダー連携の環境変数：設定済み（configured）
- **確認できたこと**：
  - 「📅 Googleカレンダーの今日の予定を見る」ボタンが表示された。
  - ユーザーがボタンをタップした。
  - 「Googleカレンダー（今日）」パネルが表示された。
  - 終日イベントが表示された（例：「終日 日建」）。
  - 時間指定イベントが表示された（例：「00:19〜00:19 とことん予定」）。
- **結論**：読み取り専用の「今日の予定」表示は本番モバイル UI で正しく動作する。
- **メモ（挙動の性質）**：
  - **読み取り専用**（read-only）。カレンダーへの書き込み・更新・削除は行わない。
  - **ユーザーのタップ起点のみ**（マウント時・自動取得・定期更新はしない。OAuth ポップアップはタップ後のみ）。
  - 取得したイベントは **Supabase / localStorage / その他ストレージに一切保存しない**（React state のみ）。
  - 既存の予定 作成／更新／書き出し（create / update / export）フローは**変更していない**。
  - Google イベント ID は引き続き**ローカルに保存していない**（重複判定は Google 側 `extendedProperties` 参照のまま）。
- **関連コミット**：`1e420fd Add read-only Google Calendar today display to reservations`

### CAL5R：デスクトップ 読み取り専用「今日の予定」表示（desktop read-only today display）

- **検証日**：2026-07-04
- **環境**：
  - Vercel 本番（production）
  - デスクトップ予約画面（`components/DesktopSchedules.tsx`）
  - Google カレンダー連携の環境変数：設定済み（configured）
- **確認できたこと**：
  - デスクトップの「Googleカレンダー（今日）」カードが表示された。
  - 「📅 Googleカレンダーの今日の予定を見る」ボタンが表示された。
  - ユーザーが本番デスクトップで動作 OK を確認した。
- **結論**：読み取り専用の「今日の予定」表示は本番デスクトップ UI でも正しく動作する。
- **メモ（挙動の性質）**：
  - **読み取り専用**（read-only）。カレンダーへの書き込み・更新・削除は行わない。
  - **ユーザーのクリック起点のみ**（マウント時・自動取得・定期更新はしない。OAuth ポップアップはクリック後のみ）。
  - 取得したイベントは **Supabase / localStorage / その他ストレージに一切保存しない**（React state のみ）。
  - 既存の予定 作成／更新／書き出し（create / update / export）フローは**変更していない**。
  - モバイル（CAL3）とデスクトップ（CAL5）の両方で読み取り表示を確認済み。
- **関連コミット**：`8f01b0b Add read-only Google Calendar today display to desktop schedules`

### CAL7R：モバイル 今日／明日／今週 読み取り表示（mobile today/tomorrow/this-week read display）
...
- **環境**：
  ...
  - Google カレンダー連携の環境変数：設定済み（configured）
- **確認できたこと**：
  - 見出し「📅 Googleカレンダーの予定」が表示された。
  - 「今日」「明日」「今週」ボタンが表示された。
  - 各範囲をタップして予定を表示できた。
  - 「今週」表示では該当行に日付プレフィックス（例：`M/D(曜)`）が付いた。
  - ユーザーが本番モバイルで動作 OK を確認した。
- **結論**：読み取り専用の今日／明日／今週表示は本番モバイル UI で正しく動作する。
- **メモ（挙動の性質）**：
  - **読み取り専用**（read-only）。カレンダーへの書き込み・更新・削除は行わない。
  - **ユーザーのタップ起点のみ**（マウント時・自動取得・定期更新はしない。範囲切り替えも都度タップ）。
  - 取得したイベントは **Supabase / localStorage / その他ストレージに一切保存しない**（React state のみ）。
  - 既存の予定 作成／更新／書き出し（create / update / export）フローは**変更していない**。
  - デスクトップ側の範囲対応（CAL8）は未実装。
- **関連コミット**：`3edc8f6 Extend mobile Google Calendar read display to tomorrow and this week`

### CAL8R：デスクトップ 今日／明日／今週 読み取り表示（desktop today/tomorrow/this-week read display）
...
- **確認できたこと**：
  ...
  - 「今週」表示では該当行に日付プレフィックス（例：`M/D(曜)`）が付いた。
  - ユーザーが本番デスクトップで動作 OK を確認した。
- **結論**：読み取り専用の今日／明日／今週表示は本番デスクトップ UI でも正しく動作する。
- **メモ（挙動の性質）**：
  - **読み取り専用**（read-only）。カレンダーへの書き込み・更新・削除は行わない。
  - **ユーザーのクリック起点のみ**（マウント時・自動取得・定期更新はしない。範囲切り替えも都度クリック）。
  - 取得したイベントは **Supabase / localStorage / その他ストレージに一切保存しない**（React state のみ）。
  - 既存の予定 作成／更新／書き出し（create / update / export）フローは**変更していない**。
  - モバイル（CAL7）とデスクトップ（CAL8）の両方で今日／明日／今週表示を確認済み。
- **関連コミット**：`15ec302 Extend desktop Google Calendar read display to tomorrow and this week`

### CAL12R：予定詳細 Googleカレンダー削除アクション（reservation detail Google Calendar delete）

- **検証日**：2026-07-04
- **環境**：
  - Vercel 本番（production）
  - 予定詳細画面（`app/reservations/[id]/page.tsx`）
  - Google カレンダー連携の環境変数：設定済み（configured）
- **確認できたこと**：
  - 「📅 Googleカレンダーから削除」ボタンが表示された。
  - 確認モーダル「Googleカレンダーからこの予定を削除しますか？」が表示された。
  - モーダルの注記「MyBrainの予定は削除されません。」が表示された。
  - Google カレンダー側のイベント（コピー）が削除された。
  - MyBrain の予定は残ったまま（削除されない）。
  - ユーザーが本番で動作 OK を確認した。
- **結論**：Googleカレンダー削除アクションは本番で正しく動作する（カレンダーのコピーだけを削除し、MyBrain の予定は残る）。
- **メモ（挙動の性質）**：
  - **手動のみ**（automatic delete はしない）。**ユーザーのクリック起点のみ**（OAuth はクリック後）。
  - **確認モーダル必須**（`window.confirm` は使わず、既存のモーダルパターンを踏襲）。
  - Google カレンダーの削除は **MyBrain の予定を削除しない**（`performDelete` / `deleteReservation` は呼ばない）。
  - Google のイベント ID は **Supabase / localStorage に保存しない**（削除対象は `extendedProperties.private.mybrainReservationId` 検索で特定）。
  - 404/410（既に無い）は成功扱い。
  - 既存の予定 作成／更新／書き出し（create / update / export）フローは**変更していない**。
- **関連コミット**：
  - `7b0c4c7 Add Google Calendar delete helpers (UI non-connected)`
  - `03eb1b0 Add Google Calendar delete action to reservation detail`
