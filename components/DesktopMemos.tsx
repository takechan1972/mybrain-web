      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex flex-1 gap-6 overflow-hidden px-8 py-5">
          ...
          <div className="flex-1 overflow-y-auto pr-1">
            {/* 一括エクスポート（一覧表示のみ）。選択したメモをZIP（Obsidian用Markdown）でまとめて書き出す。 */}
            {mode === 'list' && (
              <div className="mb-3 rounded-2xl border border-[#E8EAF3] bg-white px-4 py-3">
                ...
                <p className="mt-1 text-[12px] leading-relaxed" style={{ color: MUTED }}>
                  選択したメモをObsidian用Markdownとして、1つのZIPファイルでダウンロードできます。対応ブラウザではフォルダへ、設定済みの場合はGoogle Driveへも手動で書き出せます。
                </p>
            ...
            {/* 読み込み中の Google Drive 参照メモ・トレイ（OBS35・表示のみ）。
                参照が1件でもあれば検索語の有無に関係なく常時表示する。driveRefMemos・remove/clear を使うだけで
                持ち方（メモリのみ・保存しない）や AI 送信ロジックは一切変更しない。 */}
            {mode === 'list' && driveRefMemos.length > 0 && (
              <div className="mb-4 rounded-2xl border border-dashed p-4" style={{ borderColor: '#C9B8F5', backgroundColor: '#FAF8FF' }}>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[13px] font-bold" style={{ color: NAVY }}>
                    読み込み中のGoogle Drive参照メモ（{driveRefMemos.length}件）
                  </p>
                  <button
                    type="button"
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex min-w-0 items-center gap-2">
                            <span
                              ...
                              style={usedByAi ? { backgroundColor: LAVENDER, color: PURPLE } : { backgroundColor: '#F1F2F6', color: '#8A94A6' }}>
                              {usedByAi ? 'AIで使用' : 'AIには渡りません'}
                            </span>
                            <p className="truncate text-[13px] font-bold" style={{ color: NAVY }}>{r.title || r.fileName}</p>
                          </div>
                          <button
                            type="button"
              ...
              <div className="flex flex-col gap-4 rounded-3xl border border-[#E8EAF3] bg-white p-6 shadow-[0_10px_28px_rgba(31,53,104,0.05)]">
                ...
                <div className="mt-1 flex flex-col gap-4 rounded-2xl border border-[#E5DDFB] p-5" style={{ backgroundColor: '#FAF9FF' }}>
                  ...
                  {driveRefMemos.length > 0 && (
                    <div className="rounded-xl px-3 py-2" style={{ backgroundColor: LAVENDER }}>
                      <p className="text-[11px] font-semibold leading-relaxed" style={{ color: NAVY }}>
                        Google Drive参照 {Math.min(driveRefMemos.length, DRIVE_REF_AI_MAX_ITEMS)}件も参考にします（MyBrainには保存されません）
                      </p>
                      <p className="mt-1 text-[11px] leading-relaxed" style={{ color: '#54607A' }}>
