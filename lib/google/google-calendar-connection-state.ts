/**
 * Google カレンダー連携の接続状態を表すスケルトン（UI 非接続・OAuth 未実装）。
 *
 * - 現状は「設定があるか」までしか判定できない。OAuth / トークン保存が未実装のため、
 *   実際に接続済みかどうか（'connected'）はまだ判定できない。
 * - ここでは OAuth を始めない・トークンを保存しない・Google API も呼ばない。
 * - 'connected' は、カレンダー用 OAuth とトークン取得を実装したあとに、実トークン/セッションを見て返す。
 * - 設計方針：docs/google-calendar-integration-design.md
 */

import { isGoogleCalendarConfigured } from './google-calendar-config';

/** Google カレンダー連携の接続状態。 */
export type GoogleCalendarConnectionState =
  | 'unconfigured' // 連携に必要な公開設定が無い（クライアント ID 未設定 / 無効化）
  | 'disconnected' // 設定はあるが、まだ接続（OAuth 同意）していない
  | 'connected'; // 接続済み（※カレンダー用 OAuth / トークン取得の実装後に使う）

/** 接続状態の結果。将来トークン情報などを足せるよう object で返す。 */
export interface GoogleCalendarConnectionStatus {
  state: GoogleCalendarConnectionState;
}

/**
 * Google カレンダー連携の現在の接続状態を返す（スケルトン）。
 *
 * - 公開設定が無い → 'unconfigured'。
 * - 設定はあるが OAuth / トークン接続が未実装 → 'disconnected'。
 * - 'connected' はまだ返さない（実トークン/セッションのソースが無いため）。
 *   カレンダー用 OAuth とトークン取得を実装したら、ここで実際の接続有無を見て 'connected' を返す。
 */
export function getGoogleCalendarConnectionState(): GoogleCalendarConnectionStatus {
  if (!isGoogleCalendarConfigured()) return { state: 'unconfigured' };

  // TODO(Calendar OAuth/token 実装後): 保存済みトークン/セッションを確認し、あれば 'connected' を返す。
  // 現状はトークンの保存・取得が無いため、設定済みでも未接続として扱う。
  return { state: 'disconnected' };
}
